        ...
        {([1, 2, 3] as const).map((n, idx) => (
          <li key={n} className="flex items-center gap-4 sm:gap-10">
            <div className="shrink-0">
              <span className="flex justify-center items-center bg-[#FF4100] border border-[#24242424] rounded-full w-12 h-12 sm:w-14 sm:h-14 text-sm sm:text-base font-bold">
                {locale === "en"
                  ? `0${n}`
                  : format.number(n, {
            ...
            <div className="space-y-2 min-w-0">
              ...
              <p className="leading-relaxed text-sm sm:text-base">{t(`li-${n}-desc`)}</p>
            </div>
          </li>
        ))}
      </ul>
